    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.ADMIN_ONLY:
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply_sticker("CAACAgUAAx0CaKQl0AACAiVhghVQvCg8hFgW5od1rkPxf3JUkAACXgMAAid5aFV4tDlflYBJiSEE")
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgUAAx0CaKQl0AACAiphgheq7DBb7hButd2Wc9I_NO6FwQAC-QEAAl55aVU_tYhG2GasHiEE", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/subin_works')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('Request Support', url='https://t.me/AnimeFunChat'),
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/aincradmetwork'),
            ]
            ]
        await m.reply("<b>You can't use music features in this group, all the features of the group mamagemet will work naturally but you will have to notify my devs or my hubby at [Solo Guild](t.me/sologuild).</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
